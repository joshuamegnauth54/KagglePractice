...
```{r naive_glm}
# I had problems with y as a logical or factor.
thres_to_class <- function(data, threshold = THRESHOLD) {
  as_factor(if_else(data > threshold, 1, 0))
}

admissions <- admissions %>%
  mutate(y_admit = thres_to_class(prob_admit)) %>%
  select(-prob_admit)
...
```{r basic_model_test}
basic_preds <- thres_to_class(predict(basic_mod, type = "response"))
confusionMatrix(admissions$y_admit, reference = basic_preds, positive = "1")
```
