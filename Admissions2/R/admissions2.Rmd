...
The Area Under the Curve or AUC is basically an integration. Or, actually, I'm pretty sure AUC is, in fact, just integration. Anywho, AUC shows how much area is under the ROC curve(s), as the name states explicitly. Perfect models would have AUCs very close to one. AUCs and ROCs are extremely useful for comparing two classifiers. Each of the models defined above perform similarly; they are basically separated by less than 0.01 in terms of AUC. The ROC curves show similar performance as well, but the models notable have gains in different areas.
